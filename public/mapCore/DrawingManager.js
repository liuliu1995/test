...
var BMAP_DRAWING_MARKER = "marker",     // 鼠标画点模式
    ...
    BMAP_DRAWING_CIRCLEEX = "CircleEx";//滑动画图
...
(function () {
    ...
    (function () {
         ...
         * @remark
         * 使用者应保证提供的className合法性，不应包含不合法字符，className合法字符参考：http://12.99.109.52:8082/www.w3.org/TR/CSS2/syndata.html。
         * @shortcut addClass
         * @meta standard
         *
         * @returns {HTMLElement} 目标元素
    ...
    var DrawingManager =
         ...
         * <br />"<b>markerOptions</b>" : {CircleOptions} 所画的点的可选参数，参考api中的<a href="http://12.99.109.52:8082/developer.baidu.com/map/reference/index.php?title=Class:%E6%80%BB%E7%B1%BB/%E8%A6%86%E7%9B%96%E7%89%A9%E7%B1%BB">对应类</a>
         * <br />"<b>circleOptions</b>" : {CircleOptions} 所画的圆的可选参数，参考api中的<a href="http://12.99.109.52:8082/developer.baidu.com/map/reference/index.php?title=Class:%E6%80%BB%E7%B1%BB/%E8%A6%86%E7%9B%96%E7%89%A9%E7%B1%BB">对应类</a>
         * <br />"<b>polylineOptions</b>" : {CircleOptions} 所画的线的可选参数，参考api中的<a href="http://12.99.109.52:8082/developer.baidu.com/map/reference/index.php?title=Class:%E6%80%BB%E7%B1%BB/%E8%A6%86%E7%9B%96%E7%89%A9%E7%B1%BB">对应类</a>
         * <br />"<b>polygonOptions</b>" : {PolygonOptions} 所画的多边形的可选参数，参考api中的<a href="http://12.99.109.52:8082/developer.baidu.com/map/reference/index.php?title=Class:%E6%80%BB%E7%B1%BB/%E8%A6%86%E7%9B%96%E7%89%A9%E7%B1%BB">对应类</a>
         * <br />"<b>rectangleOptions</b>" : {PolygonOptions} 所画的矩形的可选参数，参考api中的<a href="http://12.99.109.52:8082/developer.baidu.com/map/reference/index.php?title=Class:%E6%80%BB%E7%B1%BB/%E8%A6%86%E7%9B%96%E7%89%A9%E7%B1%BB">对应类</a>
         *
         * @example <b>参考示例：</b><br />
    ...
    DrawingManager.prototype.setDrawingMode = function (drawingType) {
        //与当前模式不一样时候才进行重新绑定事件
        if (this._drawingType != drawingType || drawingType == "CircleEx") {
            closeInstanceExcept(this);
            this._setDrawingMode(drawingType);


        }
        if (drawingType != "CircleEx") {
            this._controlContinuity = true;
            this._clearAction();
        }
    }
    ...
    DrawingManager.prototype._initialize = function (map, opts) {
         ...
         * @type {DrawingType}
         */
        this._controlContinuity = true; //控制连续绘图
        //this._controlPng = true;
        this._drawingMultiple = opts.drawingMultiple || false;//是否开启多次绘制
        this._drawingCover = opts.drawingCover || false;//是否覆盖绘制
        this._flag = this._checkBrowserType(); //pc端和移动端跑判断
        this._mousedownAction = null;
        this._mousemoveAction = null;
        this._mouseupAction = null;
        // 添加的overly集合
        this.overlays = [];
    ...
    DrawingManager.prototype._bindPolylineOrPolygon = function () {
        ...
        var startAction = function (e) {
            ...
            lab = me._addLabel(e.point, "<div>双击结束</div>")
            labs.push(lab)
            points.push(e.point);
        ...
        var dblclickAction = function (e) {
            ...
            //map.setMeMapValue("addDrawingListener",polygon.getPath());
            points.length = 0;
            drawPoint.length = 0;
            me.close();
        }
     ...
     * 判断pc和移动端
     */
    DrawingManager.prototype._checkBrowserType = function () {
     ...
     * 删除滑动画圈事件
     */
    DrawingManager.prototype._clearAction = function (e) {
        var me = this,
            map = this._map;
        ...
        // me.close();
        if (this._flag) {
            map.removeEventListener('mousedown', me._mousedownAction);
            map.removeEventListener('mousemove', me._mousemoveAction);
            map.removeEventListener('mouseup', me._mouseupAction);
        } else {
            map.removeEventListener('touchstart', me._mousedownAction);
            map.removeEventListener('touchmove', me._mousemoveAction);
            map.removeEventListener('touchend', me._mouseupAction);
        }
    }
     ...
     * 外部取消绘图事件
     */
    DrawingManager.prototype._outerCloseDrawing = function (type) {
        var me = this;
        me._reset();
    }
    ...
    DrawingManager.prototype._reset = function () {
        var me = this,
            map = this._map;
        me._controlContinuity = true;
        me.close();
        me._clearAction();
        // 清空历史图形
        for (var i = 0, len = me.overlays.length; i < len; i++) {
            map.removeOverlay(me.overlays[i]);
        }
    };
     ...
     * 滑动画圆，1015
     */
    DrawingManager.prototype._bindCircleEx = function () {
        var me = this,
            map = this._map,
            mask = this._mask,
            overlay = null,
        ...
        //开始画圈绑定事件
        me._mousedownAction = function (e) {
        ...
        me._mousemoveAction = function (e) {
            //处于鼠标按下状态,才能进行画操作
            if (isMouseDown) {
                ...
                me.overlays.push(polylineOverlay);
                //setMeMapValue()"addDrawingListener"
                //更新上次画线条
                lastPolyLine = polylineOverlay
            }
        }

        var clearAction = function (e) {
            map.enableDragging(); // 启用地图拖拽
            map.enableScrollWheelZoom();
            map.enableDoubleClickZoom();
            map.enableKeyboard();
        ...
        me._mouseupAction = function (e) {
            ...
            if (isInDrawing && isMouseDown) {
                ...
                for (var i = 0; i < polygon.getPath().length; i++) {
                    var lng = polygon.getPath()[i].lng;
                    var lat = polygon.getPath()[i].lat;
                    if (polygon.getPath().length - 1 == i) {
                        str += lng + "," + lat;
                    } else {
                        str += lng + "," + lat + ",";
                    }
                }
                me.mapCore.setMeMapValue("addDrawingManager", {"type": "polygon","point":str});
                //保存多边形,用于后续删除该多边形
                polygonAfterDraw = polygon//map.meMapValue
                //alert(JSON.stringify(polyPointArray));
                if (!me._drawingMultiple) {
                    clearAction();
                    me._drawingTool.setStyleByDrawingMode("hander");
                    me._controlContinuity = true;
        ...
        isInDrawing = true;
        if (me._flag) {
            map.addEventListener('mousedown', me._mousedownAction);
            map.addEventListener('mousemove', me._mousemoveAction);
            map.addEventListener('mouseup', me._mouseupAction);
        } else {
            map.addEventListener('touchstart', me._mousedownAction);
            map.addEventListener('touchmove', me._mousemoveAction);
            map.addEventListener('touchend', me._mouseupAction);
        }
        if (me._controlContinuity) {
            setTimeout(function () {
                me._map.enableDoubleClickZoom();
            }, 2000);
            me._controlContinuity = false;
        } else {
            clearAction();
            me._controlContinuity = true;
        }
    ...
    DrawingManager.prototype._bindRectangle = function () {
        ...
        var startAction = function (e) {
            ...
            map.addOverlay(polygon);
            me.overlays.push(polygon);
            var str = {"type": "rectangle", "point": polygon.getPath().toString(), "provider": "baidu"};
            // me._map.setMeMapValue("addDrawingManager", str);
            mask.enableEdgeMove();
            mask.addEventListener('mousemove', moveAction);
            baidu.on(document, 'mouseup', endAction);
        }
	 ...
	 *
	 */
	DrawingManager.prototype.initDrawCircle = function (options) {
		if(options && options.isInitDraw) {
			var point = new BMap.Point(options.point[0], options.point[1]);
			var circle = new BMap.Circle(point, options.radius, {
				strokeColor: options.strokeColor || "blue",
				strokeOpacity: options.strokeOpacity || 0.5,
				strokeWeight: options.strokeWeight || 2
			});
			this._map.addOverlay(circle);
			this.setDrawingMode(BMAP_DRAWING_CIRCLE);
			var calculate = this._calculate(circle, point);
			this._dispatchOverlayComplete(circle, calculate);
    ...
    DrawingManager.prototype._addGeoUtilsLibrary = function () {
        if (!BMapLib.GeoUtils) {
            ...
            script.setAttribute("src", 'http://12.99.109.52:8082/api.map.baidu.com/library/GeoUtils/1.2/src/GeoUtils_min.js');
    ...
    DrawingTool.prototype._generalHtml = function (map) {
        ...
        tips[BMAP_DRAWING_CIRCLEEX] = "滑动绘图";
    ...
    DrawingTool.prototype._bind = function (panel) {
        ...
        baidu.on(this.panel, 'click', function (e) {
            ...
            //me.setStyleByDrawingMode(drawingType); //20191024删除
    ...
    DrawingTool.prototype.setStyleByDrawingMode = function (drawingType) {
        ...
        if (drawingType == "CircleEx") {
            for (var i = 0, len = boxs.length; i < len; i++) {
                ...
                if (me._controlPng && box.getAttribute('drawingType') == "CircleEx") {
                    var classStr = "BMapLib_box BMapLib_" + drawingType + "_hover";
                    if (i == len - 1) {
                        classStr += " BMapLib_last";
                    }
                    box.className = classStr;
                    me._controlPng = false;
                    break;
                } else if (box.getAttribute('drawingType') == "CircleEx") {
    ...
    DrawingTool.prototype._bindEventByDraingMode = function (drawingType) {
        ...
        //点在拖拽地图的按钮上 || drawingType == "CircleEx"
    ...
    DrawingManager.prototype._outerClickDrawing = function (drawingType) {
        this.setDrawingMode(drawingType);
        //this.setDrawingMode(drawingType);
        this.open();
    }
