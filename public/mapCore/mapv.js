(function (global, factory) {
  ...
  */
      ...
      draw: function draw(context, dataSet, options) {
          ...
          for (var i = 0, len = data.length; i < len; i++) {
              ...
              if(options.offset != undefined){
               	 context.drawImage(data[i].icon, coordinates[0]+options.offset[0], coordinates[1]+options.offset[1]);
               	}
              else
  ...
  var drawRouteDirection = {
	  draw: function draw(context, dataSet, options) {
			var data = dataSet.get(); 
		/*	context.fillStyle = options.line.fillStyle;
			context.textAlign = options.line.textAlign;
			context.textBaseline = options.line.textBaseline;
			context.strokeStyle = options.line.strokeStyle;
			context.lineWidth = options.line.lineWidth;
			context.lineCap = options.line.lineCap;
			*/
			for (var i = 0, len = data.length; i < len - 1; i++) {
				var coordinates = data[i].geometry._coordinates || data[i].geometry.coordinates;	            
				var coordinatesNext = data[i+1].geometry._coordinates || data[i+1].geometry.coordinates;
					drawDirectionLine(context,
						(coordinates[0] + (7*(coordinatesNext[0]- coordinates[0]))/21), 
						(coordinates[1] + (7*(coordinatesNext[1]-coordinates[1]))/21),
						(coordinates[0] + (9*(coordinatesNext[0]- coordinates[0]))/21),
					 	(coordinates[1] + (9*(coordinatesNext[1]-coordinates[1]))/21),
					 	options);
					drawDirectionLine(context,
						(coordinates[0] + (14*(coordinatesNext[0]- coordinates[0]))/21), 
						(coordinates[1] + (14*(coordinatesNext[1]-coordinates[1]))/21),
						(coordinates[0] + (16*(coordinatesNext[0]- coordinates[0]))/21),
					 	(coordinates[1] + (16*(coordinatesNext[1]-coordinates[1]))/21),
					 	options);
				};
	      }
	  }; 
  var drawDirectionLine = function(ctx,x1, y1, x2, y2,options) {
		var angle = Math.abs(Math.atan((x2 - x1) / (y2 - y1))); //倾斜角余角
		//var lineLength = Math.sqrt(Math.pow((x1-x2),2)+Math.pow((y1-y2),2))/30;
		var length = 10;//lineLength;//>= 10 ? 10 : lineLength ; //箭头斜线长度
		var degree = Math.PI / 10; //箭头倾斜角
		var theta = 0;
		var altha = 0;
		var a1 = 0;
		var b1 = 0;
		...
		ctx.strokeStyle = options.directLine.strokeStyle;//'#FFFF00';
		ctx.lineWidth = options.directLine.lineWidth;//1;
		ctx.lineCap = options.directLine.lineCap;//'square';
  ...
  function Layer(map, dataSet, options) {
      ...
      canvasLayer.canvas.height = canvasLayer.canvas.clientHeight;
      canvasLayer.canvas.width = canvasLayer.canvas.clientWidth;
      ...
      function update(time) {
          ...
          switch (self.options.draw) {
              ...
              case 'icon':
                  ...
                  break;
              case 'routedirection':
                  drawRouteDirection.draw(context, new DataSet(data), self.options);
                  break;
              default:
